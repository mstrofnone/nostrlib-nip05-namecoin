...
// as a chained check. It is intentionally cheap: callers use it as a
...
// IsDotBit is an alias for IsValidIdentifier, kept for callers that
// prefer the more descriptive name.
func IsDotBit(identifier string) bool { return IsValidIdentifier(identifier) }

// ResolveToJSONValue resolves a Namecoin identifier to the raw JSON
// value stored against the underlying `d/` or `id/` name on the
// Namecoin blockchain. It is useful for callers that want to inspect
// fields other than `nostr` (e.g. `bitcoin`, `lightning`, `http`).
//
// The returned slice is the raw name value as bytes; it is not
// validated beyond `QueryIdentifier`'s name-show step.
func ResolveToJSONValue(ctx context.Context, identifier string) ([]byte, error) {
	parsed := parseIdentifier(identifier)
	if parsed == nil {
		return nil, fmt.Errorf("namecoin: not a Namecoin identifier: %q", identifier)
	}
...
func QueryIdentifier(ctx context.Context, identifier string) (*nostr.ProfilePointer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("namecoin: invalid pubkey %q in name value: %w", pubkeyHex, err)
	}
...
func extractNostrFromValue(valueJSON string, parsed *parsedIdentifier) (string, []string, error) {
	...
	if err := json.Unmarshal([]byte(valueJSON), &root); err != nil {
		return "", nil, fmt.Errorf("namecoin: name value is not valid JSON: %w", err)
	}
	nostrRaw, ok := root["nostr"]
	if !ok {
		return "", nil, errors.New("namecoin: name value has no \"nostr\" field")
	}
	...
	if err := json.Unmarshal(nostrRaw, &asString); err == nil {
		if parsed.isDomain && parsed.localPart != "_" {
			return "", nil, fmt.Errorf("namecoin: simple nostr field only supports root lookup, got local-part %q", parsed.localPart)
		}
		if !hexPubKeyRegex.MatchString(asString) {
			return "", nil, errors.New("namecoin: nostr field is not a 32-byte hex pubkey")
		}
	...
	if err := json.Unmarshal(nostrRaw, &asObject); err != nil {
		return "", nil, fmt.Errorf("namecoin: nostr field is neither string nor object: %w", err)
	}
...
func extractFromDomainNamesObject(obj map[string]json.RawMessage, parsed *parsedIdentifier) (string, []string, error) {
	...
	if !ok {
		return "", nil, errors.New("namecoin: extended nostr object lacks \"names\"")
	}
	var names map[string]string
	if err := json.Unmarshal(namesRaw, &names); err != nil {
		return "", nil, fmt.Errorf("namecoin: parse names map: %w", err)
	}
	...
	if pickedPubkey == "" {
		return "", nil, fmt.Errorf("namecoin: no valid pubkey for local-part %q", parsed.localPart)
	}
...
func extractFromIdentityObject(obj map[string]json.RawMessage, parsed *parsedIdentifier) (string, []string, error) {
	...
	return "", nil, errors.New("namecoin: id/ nostr object has no valid pubkey")
}
