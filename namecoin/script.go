...
func parseNameScript(script []byte) (name string, value string, err error) {
	if len(script) == 0 || script[0] != opNameUpdate {
		return "", "", errors.New("namecoin: script is not a NAME_UPDATE")
	}
	pos := 1
...
func readPushData(script []byte, pos int) ([]byte, int, error) {
	if pos >= len(script) {
		return nil, 0, errors.New("namecoin: truncated script")
	}
	op := script[pos]
	...
	case op < opPushData1:
		...
		if end > len(script) {
			return nil, 0, errors.New("namecoin: push length exceeds script")
		}
		return script[pos+1 : end], end, nil

	case op == opPushData1:
		if pos+2 > len(script) {
			return nil, 0, errors.New("namecoin: truncated OP_PUSHDATA1")
		}
		length := int(script[pos+1])
		end := pos + 2 + length
		if end > len(script) {
			return nil, 0, errors.New("namecoin: OP_PUSHDATA1 length exceeds script")
		}
		return script[pos+2 : end], end, nil

	case op == opPushData2:
		if pos+3 > len(script) {
			return nil, 0, errors.New("namecoin: truncated OP_PUSHDATA2")
		}
		length := int(script[pos+1]) | int(script[pos+2])<<8
		end := pos + 3 + length
		if end > len(script) {
			return nil, 0, errors.New("namecoin: OP_PUSHDATA2 length exceeds script")
		}
		return script[pos+3 : end], end, nil

	case op == opPushData4:
		if pos+5 > len(script) {
			return nil, 0, errors.New("namecoin: truncated OP_PUSHDATA4")
		}
		length := int(script[pos+1]) |
			int(script[pos+2])<<8 |
			int(script[pos+3])<<16 |
		...
		if end < 0 || end > len(script) {
			return nil, 0, errors.New("namecoin: OP_PUSHDATA4 length exceeds script")
		}
		return script[pos+5 : end], end, nil

	default:
		return nil, 0, errors.New("namecoin: unsupported push opcode")
	}
}
