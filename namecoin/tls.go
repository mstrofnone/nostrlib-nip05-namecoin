...
func tlsConfigFor(server ElectrumxServer) *tls.Config {
	...
	cfg := &tls.Config{
		...
		VerifyPeerCertificate: func(rawCerts [][]byte, _ [][]*x509.Certificate) error {
			if len(rawCerts) == 0 {
				return errors.New("namecoin: no peer certificates presented")
			}
			certs := make([]*x509.Certificate, 0, len(rawCerts))
			for _, raw := range rawCerts {
				c, err := x509.ParseCertificate(raw)
				if err != nil {
					return fmt.Errorf("namecoin: parse peer cert: %w", err)
				}
			...
			return fmt.Errorf("namecoin: peer cert for %s is not in system or pinned trust store", server.Host)
		},
	}
